  ...
  return (
    <main className="page-shell page-shell-top">
      <section className="hero-card detail-card">
        ...
        <AccordionSection
          title="Контроль воды"
          description="Для каждого счетчика введите начальное и конечное показание, а система сама посчитает расход за день."
          badge={`${waterHistory.length} записей`}
          defaultOpen
        >
          ...
          <AccordionSection
            ...
            description="Вводите значения по четырём счетчикам и сразу видите расход за день."
            badge="4 счётчика"
            defaultOpen
            nested
          >
          ...
          <AccordionSection
            title="Химия и заметки"
            description="Фиксируйте долив гипохлорита, антискаланта и всё, что важно по водоподготовке."
            badge="2 реагента"
            nested
          >
          ...
          <AccordionSection
            ...
            description="Короткая история по воде, химии и расходам за последние дни."
            badge={`${waterHistory.length} дней`}
            nested
          >
            <div className="table-wrap">
              <table className="data-table">
        ...
        <AccordionSection
          title="Контроль CO2"
          description="Этот блок заполняется автоматически из сохраненных смен. Остаток и расход CO2 сразу попадают в таблицу."
          badge={formatPlainValue(co2Data?.summary.latest_remaining_tons, "т")}
        >
          ...
          <AccordionSection
            ...
            description="Выберите диапазон и сразу увидите основные показатели по газу."
            badge={`${co2Data?.summary.tracked_shifts_count ?? 0} смен`}
            defaultOpen
            nested
          >
            <div className="filter-grid">
              <label className="field">
                ...
                <input
                  ...
                  className="text-input"
                  value={co2FromDate}
                  onChange={(event) => setCo2FromDate(event.target.value)}
                />
              </label>
              ...
              <label className="field">
                ...
                <input
                  ...
                  className="text-input"
                  value={co2ToDate}
                  onChange={(event) => setCo2ToDate(event.target.value)}
                />
              </label>
            </div>
          ...
          <AccordionSection
            title="История смен"
            description="Подробная таблица по каждой смене с CO2-данными за выбранный период."
            badge={`${co2Data?.rows.length ?? 0} строк`}
            nested
          >
            <div className="table-wrap">
              <table className="data-table">
